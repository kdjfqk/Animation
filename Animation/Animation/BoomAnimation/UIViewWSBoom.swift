...
extension UIView{
    ...
    fileprivate func makeScaleValue() -> CGFloat{
        return 1 - 0.7 * (CGFloat(Int(arc4random())%101 - 50)/CGFloat(50))
    }
    ...
    fileprivate func makeRandomPath(_ aLayer:CALayer) -> UIBezierPath{
        ...
        let controlPointOffSetY = layer.position.y - 0.2 * layer.frame.size.height - CGFloat(Int(arc4random())%Int(1.2 * layer.frame.size.height))
        let endPointY = layer.position.y + layer.frame.size.height/2 + CGFloat(Int(arc4random())%Int(layer.frame.size.height/2))
...
extension UIImage{
    ...
    func createARGBBitmapContextFromImage() -> CGContext{
        if aRGBBitmapContext != nil{
            ...
            let bitmapData = UnsafeMutableRawPointer.allocate(bytes: bitmapByteCount, alignedTo: 8)
            let context = CGContext(data: bitmapData,width: pixelsWidth!,height: pixelsHeitht!,bitsPerComponent: 8,bytesPerRow: bitmapBytesPerRow,space: colorSpace, bitmapInfo:  CGImageAlphaInfo.premultipliedFirst.rawValue)!
    ...
    func getPixelColorAtLocation(_ point:CGPoint) -> UIColor{
        ...
        let resData = (cgctx.data?.assumingMemoryBound(to: UInt8.self))!//UnsafePointer<UInt8>(cgctx.data)
